...
window.onload = () => {
  document.querySelector('.search-title').innerHTML = `Search results for: ${getParameterByName('search')}`;
  const loader = '<div class="loader"><i class="fa fa-spinner fa-spin"></i></div>';

  const questionsDiv = document.querySelector('.questions');
  questionsDiv.innerHTML = loader;

  const url = `${BaseUrl}/api/v1/questions?search=${getParameterByName('search')}`;
