...
class Answer {
  ...
  static createTable(callback) {
    const createTableQuery = 'CREATE TABLE IF NOT EXISTS answers (' + ' id SERIAL PRIMARY KEY,' + ' user_id integer REFERENCES users(id) ON DELETE CASCADE,' + ' question_id integer REFERENCES questions(id) ON DELETE CASCADE,' + ' body varchar(3000),' + ' accepted boolean DEFAULT false,' + ' vote_count integer DEFAULT 0,' + ' created_at timestamp DEFAULT CURRENT_TIMESTAMP,' + ' updated_at timestamp DEFAULT CURRENT_TIMESTAMP );';
