...
class Question {
  ...
  static createTable(callback) {
    const createTableQuery = 'CREATE TABLE IF NOT EXISTS questions (' + ' id SERIAL PRIMARY KEY,' + ' user_id integer REFERENCES users(id) ON DELETE CASCADE,' + ' title varchar(100),' + ' body varchar(3000),' + ' tags varchar(255) NULL,' + ' answered boolean DEFAULT false,' + ' answer_count integer DEFAULT 0,' + ' view_count integer DEFAULT 0,' + ' created_at timestamp DEFAULT CURRENT_TIMESTAMP,' + ' updated_at timestamp DEFAULT CURRENT_TIMESTAMP );';
