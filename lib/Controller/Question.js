...
function getQuestion(req, res) {
  return _Question2.default.find(req.params.questionId, data => {
    ...
    return _Answer2.default.findForQuestion(data.question.id, data2 => {
      if (!data2.status) return res.status(_config.code.serverError).json({ status: false, errors: _config.errMsg.serverError });
      const responseData = {
        status: true,
...
function getQuestions(req, res) {
  return _Question2.default.findAll(data => {
    if (!data.status) return res.status(_config.code.serverError).json({ status: false, errors: _config.errMsg.serverError });
