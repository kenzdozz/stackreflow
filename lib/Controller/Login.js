'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _bcrypt = require('bcrypt');

var _bcrypt2 = _interopRequireDefault(_bcrypt);

var _jsonwebtoken = require('jsonwebtoken');

var _jsonwebtoken2 = _interopRequireDefault(_jsonwebtoken);

var _config = require('../config');

var _User = require('../Model/User');

var _User2 = _interopRequireDefault(_User);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function validate(user) {
  const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  const msg = {};
  let status = true;

  if (!regex.test(user.email)) {
    msg.email = 'A valid email address is required.';
    status = false;
  }
  if (user.password === '') {
    msg.password = 'Password is required.';
    status = false;
  }
  return {
    status,
    errors: msg
  };
}

function login(req, res) {
  const user = {
    email: req.body.email.trim(),
    password: req.body.password
  };

  const valid = validate(user);

  if (!valid.status) {
    return res.status(_config.code.badRequest).json(valid);
  }

  return _User2.default.find(user.email, data => {
    if (!data.status) {
      return res.status(_config.code.serverError).json({ status: false, errors: _config.errMsg.serverError });
    }
    if (data.user && data.user.password && _bcrypt2.default.compareSync(user.password, data.user.password)) {
      const token = _jsonwebtoken2.default.sign({
        email: data.user.email,
        id: data.user.id
      }, _config.jwtSecret);
      const aUser = data.user;
      delete aUser.password;
      return res.status(_config.code.ok).json({ status: true, token, user: aUser });
    }
    return res.status(_config.code.badRequest).json({ status: false, errors: { email: 'Invalid email or password.' } });
  });
}

function loginRoutes(router) {
  router.post('/auth/login', login);
}

exports.default = loginRoutes;